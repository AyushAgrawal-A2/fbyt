...
const PYTH_PUSH_ORACLE: Pubkey = anchor_lang::pubkey!("pyt2F414BA6dPttK6RddPZUdHfapoBN24GL5wbrPCou");
...
fn db_dir() -> PathBuf {
    // ../app/.data/db relative to the keeper crate
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../app/.data/db")
}
fn read_coll(name: &str) -> Value {
    let p = db_dir().join(format!("{name}.json"));
    std::fs::read_to_string(p).ok().and_then(|s| serde_json::from_str(&s).ok()).unwrap_or_else(|| json!({}))
}
fn write_coll(name: &str, v: &Value) {
    let dir = db_dir();
    let _ = std::fs::create_dir_all(&dir);
    let _ = std::fs::write(dir.join(format!("{name}.json")), serde_json::to_string_pretty(v).unwrap());
}
...
fn oracle_pool_pda(program: &Pubkey, admin_pool: &Pubkey, mint: &Pubkey) -> Pubkey {
    Pubkey::find_program_address(&[ORACLE_POOL_SEED, admin_pool.as_ref(), mint.as_ref()], program).0
}
...
fn feed32(feed_id: &[u8; 66]) -> [u8; 32] {
    let s: String = feed_id.iter().take_while(|&&b| b != 0).map(|&b| b as char).collect();
...
fn token_balance(rpc: &Rpc, acct: &Pubkey) -> u128 {
    rpc.get_token_account_balance(acct).ok().and_then(|b| b.amount.parse::<u128>().ok()).unwrap_or(0)
}

struct PriceInfo { price: i128, expo: i32, oracle_pool: Pubkey, price_acct: Pubkey }
fn price_of(rpc: &Rpc, program_id: &Pubkey, admin_pool: &Pubkey, mint: &Pubkey, oracle: &OraclePool) -> Result<PriceInfo, Box<dyn std::error::Error>> {
    ...
    Ok(PriceInfo { price, expo, oracle_pool, price_acct })
}

/// Compute a fair output amount for `input_amount` from -> to at oracle mid, minus slippage.
fn fair_output(input_amount: u128, from: &PriceInfo, to: &PriceInfo, max_slippage_bps: u64) -> u128 {
    let shift = from.expo - to.expo; // demo assets are 6dp
    let mut fair = input_amount * from.price as u128;
    if shift >= 0 { fair *= 10u128.pow(shift as u32); } else { fair /= 10u128.pow((-shift) as u32); }
    fair /= to.price as u128;
    fair * (10_000 - max_slippage_bps as u128) / 10_000
}

struct Ctx { program_id: Pubkey, vault: Pubkey, vaultpool: VaultPool, admin: AdminPool }
...
fn send_swap(
    ...
    let ix = Instruction { program_id: ctx.program_id, accounts: metas, data: fbyt_vault::instruction::Swap { data }.data() };
...
fn run_once(delegate_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program_id = Pubkey::from_str(&std::env::var("FBYT_PROGRAM_ID").unwrap_or_else(|_| fbyt_vault::ID.to_string()))?;
    let delegate = read_keypair_file(delegate_path).map_err(|e| format!("read keypair {delegate_path}: {e}"))?;
    ...
    let mut bots = read_coll("bots");
    let mut orders = read_coll("botOrders");
    let bots_obj = bots.as_object().cloned().unwrap_or_default();
    ...
    for (id, b) in &bots_obj {
        ...
        if enabled && !deleted && del == delegate_pk.to_string() {
            by_vault.entry(b["vault"].as_str().unwrap_or("").to_string()).or_default().push(id.clone());
        }
    }
    let total: usize = by_vault.values().map(|v| v.len()).sum();
    println!("[keeper] {delegate_pk}: {total} enabled bot(s) across {} vault(s)", by_vault.len());

    for (vault_str, ids) in by_vault {
        let vault = match Pubkey::from_str(&vault_str) { Ok(v) => v, Err(_) => continue };
        ...
        let ctx = Ctx { program_id, vault, vaultpool: vaultpool.clone(), admin: admin.clone() };
        ...
        for id in ids {
            ...
            let do_trade = |input_mint: Pubkey, output_mint: Pubkey, input_amount: u128| -> Result<Option<Value>, Box<dyn std::error::Error>> {
                if input_amount == 0 { return Ok(None); }
                let from = price_of(&rpc, &program_id, &vaultpool.admin_pool, &input_mint, &oracle_of(&input_mint)?)?;
                let to = price_of(&rpc, &program_id, &vaultpool.admin_pool, &output_mint, &oracle_of(&output_mint)?)?;
                let output_amount = fair_output(input_amount, &from, &to, slip);
                if output_amount == 0 { return Ok(None); }
                let sig = send_swap(&program, &ctx, &delegate_pk, input_mint, output_mint, input_amount, output_amount, &from, &to)?;
                println!("  swap {input_amount} {}… -> {output_amount} {}…  {}", &input_mint.to_string()[..4], &output_mint.to_string()[..4], &sig[..12]);
                Ok(Some(json!({ "inputMint": input_mint.to_string(), "outputMint": output_mint.to_string(), "inputAmount": input_amount.to_string(), "outputAmount": output_amount.to_string(), "signature": sig })))
            };
            ...
            match kind {
                "dca" => {
                    ...
                    let amt: u128 = s["inputAmount"].as_str().unwrap_or("0").parse().unwrap_or(0);
                    if let Some(e) = do_trade(inp, out, amt)? { execs.push(e); }
                }
                "rebalance" => {
                    ...
                    let cap: u128 = s["maxTradeAmount"].as_str().unwrap_or("0").parse().unwrap_or(0);
                    let pa = price_of(&rpc, &program_id, &vaultpool.admin_pool, &a, &oracle_of(&a)?)?;
                    let pb = price_of(&rpc, &program_id, &vaultpool.admin_pool, &b, &oracle_of(&b)?)?;
                    let usd = |amt: u128, p: &PriceInfo| amt * p.price as u128 / 10u128.pow((-p.expo) as u32);
                    ...
                    if total > 0 {
                        ...
                        if va > target_a {
                            let mut size = (va - target_a) * 10u128.pow((-pa.expo) as u32) / pa.price as u128;
                            if size > cap { size = cap; }
                            if let Some(e) = do_trade(a, b, size)? { execs.push(e); }
                        } else if vb > total - target_a {
                            let mut size = (vb - (total - target_a)) * 10u128.pow((-pb.expo) as u32) / pb.price as u128;
                            if size > cap { size = cap; }
                            if let Some(e) = do_trade(b, a, size)? { execs.push(e); }
                        } else { println!("  rebalance: within band"); }
                    }
                }
                "grid" => {
                    ...
                    let amt: u128 = s["tradeAmount"].as_str().unwrap_or("0").parse().unwrap_or(0);
                    let pb = price_of(&rpc, &program_id, &vaultpool.admin_pool, &base, &oracle_of(&base)?)?;
                    let cur = pb.price;
                    let last: i128 = bot["state"]["lastGridPrice"].as_str().and_then(|v| v.parse().ok()).unwrap_or(cur);
                    let step = last * step_bps / 10_000;
                    if cur <= last - step {
                        if let Some(e) = do_trade(quote, base, amt)? { execs.push(e); }
                        new_state = Some(json!({ "lastGridPrice": cur.to_string() }));
                    } else if cur >= last + step {
                        if let Some(e) = do_trade(base, quote, amt)? { execs.push(e); }
                        new_state = Some(json!({ "lastGridPrice": cur.to_string() }));
                    } else {
            ...
            if let Some(slot) = bots.get_mut(&id) {
                let rc = slot["runCount"].as_u64().unwrap_or(0) + 1;
                slot["runCount"] = json!(rc);
                slot["lastRunAt"] = json!(now_ms());
                if let Some(st) = new_state { slot["state"] = st; }
            }
        }
    }

    write_coll("bots", &bots);
    write_coll("botOrders", &orders);
    println!("[keeper] done");
    Ok(())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let delegate_path = args.iter().find(|a| a.ends_with(".json")).cloned()
        .unwrap_or_else(|| format!("{}/../app/scripts/.keys/delegate.json", env!("CARGO_MANIFEST_DIR")));
    let interval = std::env::var("KEEPER_INTERVAL_SEC").ok().and_then(|v| v.parse().ok()).unwrap_or(30u64);
